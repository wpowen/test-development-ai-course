...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-T12 --phase baseline`; expected exit 0 and report `reports/TD-T12/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `跨租户问题没有正确文档，系统却从其他租户语料拼出一个流畅答案。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 证据不足、来源冲突或权限不明时拒答或转人工；任何未授权读取和副作用独立阻断。 The learner produces RAG 端到端 Gate、权限攻击集和人工升级 Runbook. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 真实身份链、租户隔离、策略引擎与人工 SLA 未集成验证。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
