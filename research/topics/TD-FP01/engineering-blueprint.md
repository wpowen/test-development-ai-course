...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-FP01 --phase baseline`; expected exit 0 and report `reports/TD-FP01/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `把 system、任务、输入和自评混成一段提示词，模型补写缺失规则并把自己的输出再次判为正确。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 先冻结来源、输出 Schema、独立 Eval 与停止状态，再允许模型生成候选；静态包没有原始模型输出时保持 NOT_RUN。 The learner produces 版本化 Prompt Package、适配卡和 0/1/0 运行收据. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 不同模型对指令层级、长上下文和结构化输出的实际遵循度尚未运行。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
