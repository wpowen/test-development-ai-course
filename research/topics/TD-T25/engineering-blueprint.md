# Capstone Fixture Release Candidate：工程 Blueprint
...
主测量面为 trace coverage、artifact closure、mutation kill、0→1→0 与 unknown completeness。每个指标需要单位、分母、聚合、风险维度、观测点、版本和 owner。合成 Gate 要求三个字段全部精确匹配，单个 blocker 被破坏即 exit 1；这个阈值只为 mutation sensitivity 服务，不能外推到真实业务。统计性指标仍需真实分布、重复次数和不确定性。

## Baseline failure repair

从 `site/public/materials/ai-quality-benchmark` 运行 `python3 scripts/run_lab.py --topic TD-T25 --phase baseline`，预期 0；fault 预期 1 并显示 `只交文档、聊天截图或最终绿报告，没有首次红灯、trace、owner 或 hash closure。` 对应字段；repair 预期 0。三份 JSON 的 expected 保持相同。若 fault 假绿，依次检查 topic ID、manifest、mutation 字段、report phase 与 shell 是否吞掉退出码。禁止删除用例、改变 expected 或把 blocker 降为 warning。
...
学员生成 AI Quality Engineering Fixture RC 与评审包。AI 可整理候选与运行检查；AI 不得批准阈值、例外、权限或发布。模型、真实 RAG/Agent、企业集成、人审和发布全部 NOT_RUN。
