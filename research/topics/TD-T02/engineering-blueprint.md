...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-T02 --phase baseline`; expected exit 0 and report `reports/TD-T02/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `开发者反复针对最终回归集调 Prompt，总分持续上升，真实新问题却没有改善。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 日常调试只看 development 集；Holdout 封存并限制访问，重复使用后必须记录耗损与更新计划。 The learner produces Eval dataset、数据卡、切片矩阵和 Holdout 清单. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 合成数据的分布代表性与领域标签一致性尚未通过生产样本校准。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
