...
主测量面为 pipeline completeness、reproducibility 与逐任务结果。每个指标需要单位、分母、聚合、风险维度、观测点、版本和 owner。合成 Gate 要求三个字段全部精确匹配，单个 blocker 被破坏即 exit 1；这个阈值只为 mutation sensitivity 服务，不能外推到真实业务。统计性指标仍需真实分布、重复次数和不确定性。

## Baseline failure repair

从 `site/public/materials/ai-quality-benchmark` 运行 `python3 scripts/run_lab.py --topic TD-B01 --phase baseline`，预期 0；fault 预期 1 并显示 `只按公开名次选择客服模型，却不知道工具预算、超时和 scorer。` 对应字段；repair 预期 0。三份 JSON 的 expected 保持相同。若 fault 假绿，依次检查 topic ID、manifest、mutation 字段、report phase 与 shell 是否吞掉退出码。禁止删除用例、改变 expected 或把 blocker 降为 warning。
...
学员生成 Benchmark 流水线图和审计清单。AI 可整理候选与运行检查；AI 不得批准阈值、例外、权限或发布。课程没有复现完整公开榜单或验证当前模型排名。
