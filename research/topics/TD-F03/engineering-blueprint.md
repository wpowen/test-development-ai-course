...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-F03 --phase baseline`; expected exit 0 and report `reports/TD-F03/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `只跑一次温度为零的请求后宣称系统稳定，升级推理后端时才暴露尾部失败。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 结构、权限和副作用用确定性 Gate；语义行为保存完整上下文并重复运行，报告分布而非挑选一次结果。 The learner produces 推理变量—测试设计矩阵和重复运行计划. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 不同硬件、内核、批处理与服务端路由对确定性的影响需要当前环境实测。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
