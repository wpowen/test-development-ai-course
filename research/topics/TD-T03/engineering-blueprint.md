...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-T03 --phase baseline`; expected exit 0 and report `reports/TD-T03/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `同一个模型生成答案又评价自己的权限合规性，并用高语义总分覆盖越权工具调用。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 先执行独立确定性 blocker，再执行语义评分；高风险分歧由具名业务或安全 owner 决定。 The learner produces 风险—Oracle 决策表和分歧升级记录. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 模型 Judge 在目标领域的偏差、位置效应和一致性尚未用人工双标集校准。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
