# AI Serving 韧性：限流、Timeout、Retry、Fallback 与 Degradation validation
...
协议和指标定义来自原始标准或官方文档；“先分类错误并传播 deadline；次数、总时间、Token/费用和副作用共同限制重试；fallback 必须独立评测。”属于跨来源工程推论。Unknown 保留：真实供应商配额、Retry-After、fallback 模型质量和生产恢复没有运行。
