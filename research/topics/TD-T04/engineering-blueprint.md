...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-T04 --phase baseline`; expected exit 0 and report `reports/TD-T04/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `候选只运行一次且平均得分略高，就被写成稳定提升；高风险切片的两次失败被均值吞掉。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 先声明要估计固定样例表现还是相似任务总体表现；报告逐条原始结果和不确定性，不发明通用次数。 The learner produces 重复运行报告、分布图和发布解释模板. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 教学中的五次运行不构成生产样本量建议，样本量取决于估计目标与错误成本。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
