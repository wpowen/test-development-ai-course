...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-T01 --phase baseline`; expected exit 0 and report `reports/TD-T01/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `团队先选择一个流行指标，再用平均分替代高风险退款行为的发布决定。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 任何分数都必须回答测谁、测什么风险、谁批准和失败后做什么；owner 或阈值依据缺失即阻断。 The learner produces 版本化 Eval Contract 和决策表. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 真实业务的错误成本、发布阈值和风险接受人只能由目标组织确认。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
