# TD-QP01 Independent Comparison
...
裁决：上游事实由原生 API 回读，跨系统编排使用统一信封、精确版本与幂等副作用；先跑 fixture，再做隔离 live probe。Jira webhook → 签名/时间窗/tenant gate → Inbox → REST v3 回读 issue/changelog/transitions → Basis Gate → 版本化候选 → 人工批准 → Outbox。Prompt 必须有 system/task/critic/input/Schema/eval/manifest，provider/model 保持 NOT_RUN。
