...
针对 `TD-B06`，跨来源工程综合是：内部 Benchmark 必须连接真实决策、领域 Oracle 和具名 owner。 具体工作面是 业务风险、四类内部数据、Composite Scorer、维护 owner 与治理。传统测试的 test basis、独立 Oracle、负控、回归、退出码和事故闭环仍有效；AI 场景扩张了版本图、概率性多次运行、检索/工具轨迹、Judge 校准、数据污染和成本/延迟分布。

反例 `把公开题翻译成中文就当企业发布 Gate，缺少权限、事故和业务 Oracle。` 会改变行动：先保存 manifest 和 raw record，再定位第一个破坏合同的组件；不能先改 Prompt 或平均阈值。度量 `risk coverage、blocker、slice quality、latency/cost 与 freshness` 必须声明测量点、分母、切片、聚合、版本、owner 与失败动作。高风险 blocker 不允许被许多简单样例平均抵消。

## Unknown

真实企业数据、标注团队、权限和发布连接未建立。 来源也不能给出目标公司的流量、事故频率、权限模型、可接受误报、定价、SLO、隐私分类、真实 Judge 对齐或发布 owner。它们保持 UNKNOWN/NOT_RUN。课程不能用一般方法论伪装成现场证据。
