...
From `site/public/materials/ai-foundations-eval`, run `python3 scripts/run_lab.py --topic TD-T09 --phase baseline`; expected exit 0 and report `reports/TD-T09/baseline.json`. Then run the fault phase; expected exit 1 and the report must name the mutation derived from `旧退款政策和新政策同时被分块，召回系统返回语义更相似的旧文档。`. Finally run repair; expected exit 0 with the same expected contract. The repair does not relax the Oracle; it restores the observation.
...
Professional decision: 每个 chunk 保留 source_id、版本、生效期和 ACL；冲突或过期来源不得静默进入当前索引。 The learner produces RAG corpus manifest、chunk schema 和索引准入报告. Human review must confirm policy, risk acceptance and publication language. Residual unknown: 真实文档系统的 ACL、删除 SLA、OCR 和分块质量尚未连接。 A real adapter must preserve raw model/retrieval/tool evidence and create a separate integration receipt before any maturity promotion.
