# TD-AP05 研究简报：容量曲线与瓶颈归因

## 决策问题

Controlling question：如何用阶梯负载找到第一个门禁失效点，并把瓶颈归因到队列、prefill、decode、tool 或 retry？

如何从阶梯负载找到 SLO 首次失效点，并用 queue、prefill、decode、tool 信号定位瓶颈？

- 目标学习者：已会接口测试、首次负责 Agent 性能与稳定性的测试开发工程师。
- 待做决定：当前版本在固定夹具负载下是否保持门禁。
- 必交工件：synthetic 容量曲线和瓶颈证据卡。
