# TD-QP04 Independent Comparison
...
裁决：上游事实由原生 API 回读，跨系统编排使用统一信封、精确版本与幂等副作用；先跑 fixture，再做隔离 live probe。Platform events → signature/schema Gateway → Inbox → versioned orchestrator → Outbox → adapters → notice/audit → DLQ/reconciliation → rollback。Prompt 必须有 system/task/critic/input/Schema/eval/manifest，provider/model 保持 NOT_RUN。
