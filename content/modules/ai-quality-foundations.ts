...
export const aiQualityFoundationPages: TutorialPage[] = (specs.map(makePage)).map((page): TutorialPage => ({
  ...page,
  blocks: [...composeDeepPage(page.blocks, aiFoundationsDeepBlocks(page.id), ragQualityDeepBlocks(page.id), promptFirstLessonSupplement(page.id)), ...wave2FoundationRepairBlocks(page.id)],
}));
