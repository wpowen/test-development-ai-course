...
const benchmarkWave2: Record<string, TutorialBlock> = {
...
benchmarkWave2["TD-B02"] = { title: "Dataset 分层 worked case", body: ["将客服数据按事件、实体和时间去重后，representative 保留常见 FAQ，challenge 放大退款边界，regression 固定事故，sealed holdout 限权。学员提交 Dataset Card、split hash、访问日志和争议 adjudication；迁移到 Agent 任务只替换实体和 Oracle。"], table: { headers: ["症状", "层", "动作"], rows: [["Holdout 异常高", "泄漏", "查近重复"], ["高风险过少", "抽样", "补 challenge"], ["标注争议", "Oracle", "双标裁决"], ["反复访问", "治理", "冻结轮换"]], caption: "数据角色和访问权必须一起审计。" } };
benchmarkWave2["TD-B03"] = { title: "指标分母 worked case", body: ["10 题各跑 5 次，pass@5 很高但单次成功低且一次越权被均值掩盖。学员保留逐题 ledger、超时、重试、成本和 blocker，Metric Card 写清公式、分母、缺失规则和区间；迁移到 RAG 增加引用切片，Agent 增加工具 blocker。"], table: { headers: ["症状", "层", "动作"], rows: [["pass@5 高", "重试", "报单次率与成本"], ["超时排除", "分母", "放回重算"], ["Judge 高越权", "Oracle", "一票否决"], ["差异很小", "不确定性", "报区间 UNKNOWN"]], caption: "指标是决策接口，不是排行榜数字。" } };
benchmarkWave2["TD-B06"] = { title: "公共方法迁移 worked case", body: ["把公共任务结构改成企业客服基准：加入政策版本、租户隔离、退款权限、事故回归和具名 owner，再用同预算 adapter 比较。没有真实数据、标注或发布连接时只能交付设计/fixture，不能称生产基准。"], table: { headers: ["症状", "层", "动作"], rows: [["翻译公开题", "业务 Oracle", "回链事故"], ["公共分高客服失败", "切片", "补中文风险"], ["无人维护", "治理", "补 retire owner"], ["比较不公平", "协议", "锁定预算"]], caption: "可迁移的是方法结构，不是榜单结论。" } };
benchmarkWave2["TD-T25"] = { title: "Capstone 追踪 worked case", body: ["从 PRD 风险追到 Dataset、Prompt/Schema、Composite Oracle、CI、Trace regression、性能、Waiver 和 rollback；先 hash closure，再 baseline 0，注入 fault 变 1，repair 不改 expected 回 0。提交 owner、receipt 和未知项。"], table: { headers: ["症状", "层", "动作"], rows: [["无需求映射", "traceability", "补 owner"], ["改 expected 过 fault", "Oracle", "比 hash"], ["ZIP 不一致", "closure", "逐 member 对账"], ["fixture 写成 live", "边界", "恢复 NOT_RUN"]], caption: "出口是 fixture RC，不是生产证明。" } };
benchmarkWave2["TD-B03"] = { title: "指标分母诊断", body: ["worked walkthrough：10 题各跑 5 次，pass@5 很高但单次成功低且一次越权被均值掩盖。学员保留逐题 ledger、超时、重试、成本和 blocker；迁移到 Agent 加工具 blocker，RAG 加引用切片。"], table: { headers: ["症状/问题", "疑似层", "下一步检查", "修复/重跑"], rows: [["pass@5 高单次低", "重复", "查重试与成本", "报告单次率"], ["超时被排除", "分母", "查 missing policy", "放回重算"], ["Judge 高但越权", "Oracle", "查工具 trace", "一票否决并重跑"], ["差异很小", "不确定性", "查区间与样本", "不足则 UNKNOWN"]], caption: "指标必须可回链到分母与失败动作。" } };
benchmarkWave2["TD-T25"] = { title: "Capstone 追踪诊断", body: ["worked walkthrough：从 PRD 风险追到 Dataset、Prompt/Schema、Composite Oracle、CI、Trace regression、性能、Waiver 和 rollback；先 hash closure，再 baseline 0，注入 fault 变 1，repair 不改 expected 回 0。"], table: { headers: ["症状/问题", "疑似层", "下一步检查", "修复/重跑"], rows: [["报告绿但无需求映射", "traceability", "查 requirement/risk", "补 owner 后重跑"], ["fault 通过改 expected", "Oracle", "比三阶段 hash", "恢复 expected 重跑"], ["ZIP 与公开文件不一致", "closure", "逐 member 对账", "重建 closure"], ["fixture receipt 写成 live", "边界", "查状态字段", "恢复 NOT_RUN"]], caption: "Capstone 出口是可审计 fixture RC。" } };
...
export const benchmarkCapstonePages = specs.map(buildQualityBenchmarkPage).map((page) => ({
  ...
  blocks: composeDeepPage([...page.blocks, ...(benchmarkWave2[page.id] ? [benchmarkWave2[page.id]] : [])], benchmarkDeepBlocks(page.id), qualityPlatformDeepBlocks(page.id)),
}));
